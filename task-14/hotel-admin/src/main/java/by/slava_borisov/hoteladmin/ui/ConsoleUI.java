...
public class ConsoleUI {
    ...
    public ConsoleUI() {
        ...
        log.info("ConsoleUI инициализирован");
    }
    ...
    public int readInt() {
        while (true) {
            try {
                ...
                log.debug("Некорректный ввод числа: {}", e.getMessage());
                System.out.println(Messages.INVALID_CHOICE);
                System.out.print(Messages.MENU_PROMPT);
            }
        }
    ...
    public double readDouble() {
        while (true) {
            try {
                ...
                log.debug("Некорректный ввод числа с плавающей точкой: {}", e.getMessage());
    ...
    public void start(Menu mainMenu) {
        log.info("Запуск приложения с главным меню: {}", mainMenu.getClass().getSimpleName());
        navigator.setCurrentMenu(mainMenu);
        run();
    }

    public void run() {
        log.info("Начало работы главного цикла меню");
        while (true) {
            ...
            log.info("Пользователь выбрал пункт меню: {} в {}", choice, currentMenu.getClass().getSimpleName());

            try {
                currentMenu.executeOption(choice);
                log.debug("Пункт меню {} успешно выполнен", choice);
            } catch (DuplicateRoomNumberException e) {
                log.error("Ошибка выполнения пункта меню {}: {}", choice, e.getMessage());
                System.err.println(Messages.ERROR_PREFIX + " " + e.getMessage());
            } catch (Exception e) {
                log.error("Неожиданная ошибка при выполнении пункта меню {}: {}", choice, e.getMessage(), e);
                System.err.println(Messages.ERROR_PREFIX + " " + e.getMessage());
            }
        }
    }
    ...
    public void close() {
        log.info("Закрытие ConsoleUI и Scanner");
        scanner.close();
    }
}
